...
#'
#' @export
movements <- function(data=NULL,space.use=TRUE,from.previous=TRUE,cumulative=TRUE, downstream.node=NULL,coords,lon.name="lon",lat.name="lat",id.name="id",date.time.name=NULL){
  ...
  if(is.null  (date.time.name)==FALSE){colnames(coords)[which(colnames(coords)==date.time.name)] <- "date.time"
  coords <- coords[with(coords,order(id,date.time)),]}

  #remove critical NAs
  if(length(c(which(is.na(coords$lon)==TRUE),which(is.na(coords$lat)==TRUE),which(is.na(coords$id)==TRUE))) > 0){
    coords <- coords[-c(which(is.na(coords$lon)==TRUE),which(is.na(coords$lat)==TRUE),which(is.na(coords$id)==TRUE)),]}
  ...
  down.node <- if(is.null(downstream.node)==FALSE){nodes[which(nodes$id==downstream.node),c("lon","lat")]}
  ...
  if(is.null(date.time.name)==FALSE){time.diffs.a <- vector("list",length(coords.id))}
  ...
  for(a in 1:length(coords.id)){
   ...
   message("Analyzing individal ",id.a)

    coords.a <- coords[which(coords$id==id.a),]

    if(is.null(date.time.name)==FALSE){times.a <- keep[which(keep$id==id.a),"date.time"]}
    ...
    if(is.null(date.time.name)==FALSE){time.diffs.i <- vector("list",nrow(coords.a))}
    ...
    for(i in 1:nrow(coords.a)){
      ...
      #time difference
      ################
      if(is.null(date.time.name)==FALSE){
      ...
      if(space.use==TRUE){ #do space use calculations
      ...
      ###############
      if(from.previous==TRUE | cumulative == T){
      ...
      if(is.null(downstream.node)==FALSE){
    ...
    if(space.use==TRUE){space.use.a[[a]] <- do.call("c",space.use.i)}
    if(from.previous==TRUE | cumulative == T){
    ...
    if(is.null(date.time.name)==FALSE){time.diffs.a[[a]] <- do.call("c",time.diffs.i)}

  } #closes loop over individuals

  space.use.a <- if(space.use==TRUE){do.call("c",space.use.a)}else{NULL}
  from.prev.a <- if(from.previous==TRUE | cumulative == T){do.call("c",from.prev.a)}else{NULL}
  cumm.dist.a <- if(cumulative == T){do.call("c",cumm.dist.a)}else{NULL}
  from.downstream.a <- if(is.null(downstream.node)==FALSE){do.call("c",from.downstream.a)}else{NULL}
  direction.a <- if(is.null(downstream.node)==FALSE){do.call("c",direction.a)}else{NULL}
  time.diffs.a <- if(is.null(date.time.name)==FALSE){do.call("c",time.diffs.a)}else{NULL}
  ...
  final.res <- cbind.data.frame(do.call("cbind.data.frame",temp[which(sapply(temp,is.null)==FALSE)]),coords)
