...
output:
  pdf_document:
    toc: true
vignette: >
  %\VignetteIndexEntry{SNMA Stream Network Movement Analyses Vignette}
...
It is very important that the shapefile is formatted correctly. I recommend checking it with the *str()* function. This should show that it is class 'sf' and 'data.frame' with 2 variables. The first must be a vector of segment ids, and the second must be a geometry object with X and Y (in that order). Look at the *stream.line* object as a point of comparison.

```{r}
str(stream.line)
```

**Caution:** Be very careful with the construction of your line segments. The points within segments must be consecutive going either upstream or downstream. If you create a segment by merging two existing segments, this requirement may be violated. If, for example, one segment was created going downstream and the other was created going upstream and you merge them into a single segment, then the points will be inverted for part of that new segment. This will give spurious results or make the functions fail. As a quick and dirty check, the *prep.data()* function calculates the straight line distances between each consecutive pair of points within each segment and prints the median and maximum per segment: "l segment ____ straight distance between points: median = ____ (m), max = ___ (m)" If there is either a large mismatch between the median and max or the max is much larger than you would expect, that suggests that the segment was not constructed consecutively and you may need to modify or re-make that segment.

A more comprehensive visual check can be performed by using *st_coordinates()* to extract the coordinates for each segment, then adding a column of order information and plotting with *ggplot()*. This should show a continuous color gradient within each segment. It does not matter if some segments are ordered downstream while others are ordered upstream so long as the order is consistent within each segment.

The code below illustrates this procedure with the inbuilt data set. Visualizations will be more meaningful with full data sets.
...
You should also check the output of *prep.data()*, for example, making sure that in *$dps.segments* node1 and node2 are never identical within a row. Likewise, check that *$node.distances* are never 0 or NULL.
...
To illustrate, let's use the *animal.points* data frame that comes with **SNMA**. It contains two sets of coordinates. The "raw" set contains the actual data and is the only set needed for analyses. The "shifted" points are simply to illustrate how the functions work. Let's start by plotting the raw data.
...
Here we have coordinates for two turtles. As you can see, they are within the stream boundaries, but aren't lined up with the center line object. The functions in **SNMA** will take care of that by matching each point with the closest point on the stream network using the object generated in *prep.data()*, but for illustrative purposes, let's plot roughly what that looks like using the "shifted" columns (these were generated manually and are not overly accurate; again, just for illustration).
...
Note that this package was originally released on GitHub under the name "SNM: Stream Network Movements."
